...
const HeroStats = ({ isOpen, onClose, matches = [] }) => {
  ...
  // Get sort icon with better visual indicators
  const getSortIcon = (key) => {
    if (sortConfig.key !== key) {
      return (
        <div className="flex flex-col items-center">
          <FaSortUp className="text-gray-500 text-xs" />
          <FaSortDown className="text-gray-500 text-xs -mt-1" />
        </div>
      );
    }
    return sortConfig.direction === 'asc' ? 
      <FaSortUp className="text-yellow-400 text-lg font-bold" /> : 
      <FaSortDown className="text-yellow-400 text-lg font-bold" />;
  };
       ...
       <div className="modal-box w-full max-w-7xl bg-[#23232a] rounded-2xl shadow-2xl p-8 max-h-[90vh] overflow-hidden animate-slideIn">
        ...
        <div className="overflow-x-auto" style={{ maxHeight: '45vh' }}>
          ...
          <div className="text-xs text-gray-400 mb-2 text-center">
            💡 Click any column header to sort • Active sort: <span className="text-yellow-400 font-semibold">
              {sortConfig.key === 'hero' ? 'HERO' : 
               sortConfig.key === 'pick' ? 'PICK' :
               sortConfig.key === 'pickRate' ? 'PICK RATE' :
               sortConfig.key === 'ban' ? 'BAN' :
               sortConfig.key === 'banRate' ? 'BAN RATE' :
               sortConfig.key === 'win' ? 'WIN' :
               sortConfig.key === 'winRate' ? 'WIN RATE' : 'HERO'}
            </span> 
            <span className={`ml-1 ${sortConfig.direction === 'asc' ? 'text-green-400' : 'text-red-400'}`}>
              {sortConfig.direction === 'asc' ? '⬆️ A→Z' : '⬇️ Z→A'}
            </span>
          </div>
          <table className="w-full text-sm">
            <thead className="sticky top-0 z-10">
              <tr>
                <th 
                  className={`py-3 px-4 text-left font-bold cursor-pointer transition-colors ${
                    sortConfig.key === 'hero' 
                ...
                <th 
                  className={`py-3 px-4 text-center font-bold cursor-pointer transition-colors ${
                    sortConfig.key === 'pick' 
